...
# Set paths for RGB and point frequency projection
output_rgb_dir = os.path.join(data_dir, 'indoor3d_sem_seg_rgb_projection')
output_pf_dir = os.path.join(data_dir, 'indoor3d_sem_seg_pf_projection')
if not os.path.exists(output_pf_dir):
    os.mkdir(output_pf_dir)
if not os.path.exists(output_rgb_dir):
    os.mkdir(output_rgb_dir)

output_rgb_room_filelist = os.path.join(output_rgb_dir, 'room_filelist.txt')
ouput_pf_room_filelist = os.path.join(output_pf_dir, 'room_filelist.txt')
fout_room_rgb = open(output_rgb_room_filelist, 'w')
fout_room_pf = open(ouput_pf_room_filelist, 'w')
...
for i, data_label_filename in enumerate(data_label_files):
    ...
    data, label = indoor3d_util.room2blocks_wrapper_normalized(data_label_filename, NUM_POINT, block_size=1.0, stride=0.5,
                                                 random_sample=False, sample_num=None, filename=filename)
    print(('{0}, {1}'.format(data.shape, label.shape)))
    for idx in range(data.shape[0]):
        fout_room.write(str(idx)+'_'+os.path.basename(data_label_filename)[0:-4]+'\n')

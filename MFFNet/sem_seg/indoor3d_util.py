...
import cv2
from gen_point_freq_projection import *
from gen_rgb_projection import *
...
# -----------------------------------------------------------------------------
# ARGUMENTS FOR RGB & POINT FREQUENCY PROJECTION GENERATION
# -----------------------------------------------------------------------------

PF_DATA_PATH = os.path.join(ROOT_DIR, 'data', 'indoor3d_sem_seg_pf_projection')
RGB_DATA_PATH = os.path.join(ROOT_DIR, 'data', 'indoor3d_sem_seg_rgb_projection')

config4pf_projection = {'resolution': 32,
                        'block_size': 3,
                        'other_operation': '000',
                        'alpha': 0.0007
                        }

config4rgb_projection = {'resolution': 32,
                         'block_size': 3,
                         'sample_num': 7,
                         'kernel_size': 3,
                         'radius': 0.5}
...
def mkdir4pf_projection(filename):
    ...
    if not os.path.exists(result_dir_xy):
        os.makedirs(result_dir_xy)

    if not os.path.exists(result_dir_xz):
        os.makedirs(result_dir_xz)

    if not os.path.exists(result_dir_yz):
        os.makedirs(result_dir_yz)


def mkdir4rgb_projection(filename):
    # Create a new directory if it does not exist
    ...
    # Create sub-directories for storage
    result_dir_xy1 = os.path.join(result_dir, 'xy1')
    result_dir_xz1 = os.path.join(result_dir, 'xz1')
    result_dir_yz1 = os.path.join(result_dir, 'yz1')
    result_dir_xy2 = os.path.join(result_dir, 'xy2')
    result_dir_xz2 = os.path.join(result_dir, 'xz2')
    result_dir_yz2 = os.path.join(result_dir, 'yz2')

    if not os.path.exists(result_dir_xy1):
        os.makedirs(result_dir_xy1)

    if not os.path.exists(result_dir_xz1):
        os.makedirs(result_dir_xz1)

    if not os.path.exists(result_dir_yz1):
        os.makedirs(result_dir_yz1)

    if not os.path.exists(result_dir_xy2):
        os.makedirs(result_dir_xy2)

    if not os.path.exists(result_dir_xz2):
        os.makedirs(result_dir_xz2)

    if not os.path.exists(result_dir_yz2):
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
                random_sample=False, sample_num=None, sample_aug=1, filename='none'):
    ...
    if not random_sample:
        ...
        num_block_z = int(np.ceil((limit[2] - block_size) / stride)) + 1
        for i in range(num_block_x):
            for j in range(num_block_y):
                ...
                for k in range(num_block_z):
                    xbeg_list.append(i * stride)
                    ybeg_list.append(j * stride)
                    zbeg_list.append(k * stride)

    else:
        num_block_x = int(np.ceil(limit[0] / block_size))
        num_block_y = int(np.ceil(limit[1] / block_size))
        num_block_z = int(np.ceil(limit[2] / block_size))
        if sample_num is None:
            sample_num = num_block_x * num_block_y * num_block_z * sample_aug
        for _ in range(sample_num):
            xbeg = np.random.uniform(-block_size, limit[0]) 
            ybeg = np.random.uniform(-block_size, limit[1])
            zbeg = np.random.uniform(-block_size, limit[2])
            xbeg_list.append(xbeg)
            ybeg_list.append(ybeg)
            zbeg_list.append(zbeg)

    # Create directories for storage
    mkdir4rgb_projection(filename=filename)
    mkdir4pf_projection(filename=filename)
    ...
    for idx in range(len(xbeg_list)): 
        ...
        zcond = (data[:,2]<=zbeg+block_size) & (data[:,2]>=zbeg)

        # Find points that satisfy the xy condition
        cond = xcond & ycond

        # Find points that satisfy the z condition at the same time
        cond = cond & zcond
...
def room2blocks_plus_normalized(data_label, num_point, block_size, stride,
                                random_sample, sample_num, sample_aug, filename='none'):
    """ room2block, with input filename and RGB preprocessing.
    ...
    data_batch, label_batch = room2blocks(data, label, num_point, block_size, stride,
                                          random_sample, sample_num, sample_aug, filename)
